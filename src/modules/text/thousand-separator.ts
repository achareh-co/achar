/**
 * Thousand Separator
 * @param text entry text, ex: 300000
 * @param separator separator character, ex: ,
 * @returns string formatted text, ex: 300,000
 */
export default function thousandSeparator(text: string | number, separator = ','): string {
  text = text ?? '';
  return text.toString().replace(/\B(?=(\d{3})+(?!\d))/g, separator);
}

if (import.meta.vitest) {
  const { it, expect } = import.meta.vitest;

  it('separate thousands with comma in a text', () => {
    expect(thousandSeparator('')).toBe('');
    expect(thousandSeparator(0)).toBe('0');
    expect(thousandSeparator('1')).toBe('1');
    expect(thousandSeparator(1)).toBe('1');
    expect(thousandSeparator('123')).toBe('123');
    expect(thousandSeparator('1234')).toBe('1,234');
    expect(thousandSeparator('123456')).toBe('123,456');
    expect(thousandSeparator(123456)).toBe('123,456');
    expect(thousandSeparator('1234567890')).toBe('1,234,567,890');
    expect(thousandSeparator('this a text with 1 and 12345')).toBe('this a text with 1 and 12,345');
  });

  it('separate thousands with slash in a text', () => {
    expect(thousandSeparator('1', '/')).toBe('1');
    expect(thousandSeparator('123', '/')).toBe('123');
    expect(thousandSeparator('1234', '/')).toBe('1/234');
    expect(thousandSeparator('123456', '/')).toBe('123/456');
    expect(thousandSeparator('1234567890', '/')).toBe('1/234/567/890');
    expect(thousandSeparator('this a text with 1 and 12345', '/')).toBe(
      'this a text with 1 and 12/345',
    );
  });
}
